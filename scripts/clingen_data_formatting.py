...
# Create a datframe where only certain ancestor terms exist - indentification of diseases missing from analysis
ancestor_counts = clingen_ensembl.groupby('mondo_disease_id')['mondo_ancestor_id'].nunique()
target_terms = ['MONDO_0003847', 'MONDO_0002254', 'MONDO_0020683', 'MONDO_0700007','MONDO_0005550','MONDO_0021669','MONDO_0019040']
...
only_target_diseases[['mondo_disease_id', 'disease_label', 'mondo_ancestor_id', 'ancestor_label']].drop_duplicates(keep='first').to_csv(
    os.path.join(DATA_PATH, 'missingdata', 'clingen.hereditary_syndromic_only.txt'), sep='\t', index=False
)

#remove entries with hereditary or syndromic disease classification
clingen_ensembl = clingen_ensembl.loc[~clingen_ensembl['mondo_ancestor_id'].isin(target_terms)]

#Log entries with missing MONDO ancestor IDs
...
#Save the list of MONDO ancestor IDs for lookup of equivalent terms in other ontologies
mondo_ancestor_ids = clingen_ensembl[['mondo_ancestor_id','ancestor_label']].drop_duplicates(keep = 'first').to_csv(os.path.join(DATA_PATH, 'ontologymapping', 'mondo.humandisease.clingen.txt'), sep = '\t', index = False)
