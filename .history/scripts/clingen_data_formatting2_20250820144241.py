...
def gtf_to_txt(ensembl_data, chunksize = 10000):
    ...
    for chunk in pd.read_csv(ensembl_data, names=data_header, sep='\t', skiprows=5, chunksize=chunksize):
        ...
        chunk = chunk[['seqname', 'start', 'end', 'strand', 'gene_id', 'gene_name', 'gene_biotype', 'transcript_id','transcript_support_level']]
        chunk = chunk.loc[((chunk['gene_biotype'] == 'protein_coding') & (chunk['transcript_support_level'] ==1))]
        chunk = chunk.drop(columns = ['gene_biotype', 'transcript_support_level'])
...
def main():
    ...
    print(clingen['classification'].value_counts())

    clingen.to_csv('data/clingen.formatted.txt', sep = '\t')

    ontology_mapping = clingen[['mondo_ancestor_id', 'ancestor_label']].drop_duplicates(keep = 'first').to_csv('data/ontology_mapping.starter.txt', sep = '\t')
