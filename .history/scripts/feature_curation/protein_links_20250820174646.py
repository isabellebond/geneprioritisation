...
def get_protein_links(path, clingen, strong_genes, experimental_protein_link_threshold=400):
    ...
    string = pd.read_csv(path, sep='\t', header=None)
    ...
    for ancestor in clingen['ancestor_id'].unique():
        clingen_subset = clingen[clingen['ancestor_id'] == ancestor]
        strong_genes_subset = strong_genes[strong_genes['ancestor_id'] == ancestor]
        ...
        for i,row in clingen_subset.iterrows():
            ...
            clingen.loc[((clingen['gene_id'] == row['gene_id']) & (clingen['ancestor_id'] == ancestor)), 'experimental_protein_link'] = True
            clingen.loc[((clingen['gene_id'] == row['gene_id']) & (clingen['ancestor_id'] == ancestor)), 'linked_protein_ids'] = ';'.join(gene_interactions['linked_protein_id'].unique())
            clingen.loc[((clingen['gene_id'] == row['gene_id']) & (clingen['ancestor_id'] == ancestor)), 'linked_protein_names'] = ';'.join(gene_interactions['linked_gene_name'].unique())
            clingen.loc[((clingen['gene_id'] == row['gene_id']) & (clingen['ancestor_id'] == ancestor)), 'evidence_strength'] = ';'.join(gene_interactions['experimental'].astype(str).unique())
...
def main():
    ...
    cligen_strong = clingen_strong[['gene_id', 'gene_name', 'protein_id', 'mondo_disease_id', 'disease_label', 'mondo_ancestor_id', 'ancestor_label']].drop_duplicates()

    protein_links = get_protein_links('data/protein_links.txt', cligen_strong, clingen)

    print(protein_links.head())
    protein_links.to_csv('data/featurens/protein_links.txt', sep='\t', index=False)
